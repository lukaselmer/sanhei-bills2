...
export class CombinedBillArticlesChangesExtractor {
  ...
  private extractDefinitiveArticleDeletions() {
    const articleIdsStillInUse = ([] as string[]).concat(
      this.articlesToChangeArticleId.map(a => (a.article as Article).id),
      this.articlesToCreate.map(a => a.article ? a.article.id : undefined).filter(a => a).map((a: string) => a),
      this.articlesToChange.map(a => a.article ? a.article.id : undefined).filter(a => a).map((a: string) => a)
    );

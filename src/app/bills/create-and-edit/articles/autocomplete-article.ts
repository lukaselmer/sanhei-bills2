...
export class AutocompleteArticle {
  ...
  get catalogId() {
    return this.article.catalogId;
  }
  get description() {
    return this.article.description;
  }
  get dimension() {
    return this.article.dimension;
  }
  get price() {
  ...
  get displayValue() {
    return [this.catalogId, this.description, this.dimension].filter(field => field).join(' / ');
  }

  matchesAll(filters: string[]): boolean {
    return filters.every(filter => this.stringToFilter.includes(filter));
  }
}

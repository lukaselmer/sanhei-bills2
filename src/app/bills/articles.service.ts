...
export class ArticlesService {
  ...
  async updateArticles(billId: number, updatedArticles: CombinedBillArticle[]) {
    const store = this.dataStore.store();
    ...
    for (const article of service.articlesToChangeArticleId) {
      const ba = article.billArticle as BillArticle;
      ba.articleId = parseFloat((article.article as Article).id as string);
      await this.dataStore.updateBillArticle(ba);
    }
  ...
  private buildBillArticle(billId: number, combinedArticle: CombinedBillArticle): BillArticle {
    return {
      id: 0,
      amount: parseFloat(combinedArticle.amount),
      articleId: (combinedArticle.article as Article).id as number,
      billId,
      createdAt: 0, updatedAt: 0, deletedAt: undefined
    };
  }
